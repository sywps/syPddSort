...
const FIRST_SKIN_ICON_UUID = '26679b68-2d06-321a-81d2-b42b4edc9325';
const SOURCE_IMAGE = 'assets/HomeAssetsBundle/GameUI/home_bg.jpeg';
const SOURCE_META = `${SOURCE_IMAGE}.meta`;
const TARGET_IMAGE = 'assets/BootstrapBundle/GameUI/home_bg.png';
...
const FIRST_SKIN_ICON = 'assets/GameAssetsBundle/Skins/Icons/bg_005.png';
...
assert.strictEqual(sha256(TARGET_IMAGE), sha256('assets/LevelData/Skins/Background/bg_005/background.png'), 'startup background must be exact bg_005 pixels');
assert.strictEqual(sha256(HOME_IMAGE), 'f1b6b94a91955a23262d10ab22afcb2ea502bd42b3065dcd7e599cbf30196bed', 'Home background must stay unchanged');
assert.ok(!fs.existsSync(absolute('assets/BootstrapBundle/GameUI/home_bg.jpeg')), 'old startup background must be removed');
assert.ok(!fs.existsSync(absolute('assets/GameAssetsBundle/Skins/Icons/bg_000.png')), 'retired icon must be removed');
...
assert.strictEqual(defaultSkin.iconKey, 'Skins/Icons/bg_005');
assert.ok(!skinConfig.skins.some((skin) => skin.id === 1000 || skin.id === 1001), 'retired skin 1001 must leave local config');

const skinModule = read('assets/Scripts/Core/GameCtrlModules/SkinBackgroundModule.ts');
const defaultBranchStart = skinModule.indexOf('if (equippedId === BUILTIN_BACKGROUND_SKIN_ID)');
...
assert.ok(skinModule.includes('RETIRED_BACKGROUND_SKIN_IDS = new Set<number>([1000, 1001])'), 'runtime must explicitly retire skin 1001');
...
assert.ok(!downscaleScript.includes('assets/BootstrapBundle/GameUI/home_bg.jpeg'), 'texture maintenance must not restore the old background');
