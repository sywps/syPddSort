...
assert.match(
    onCapacityAdTap,
    /runRewardedGrant\('pch_conveyor_expand',[\s\S]*?grantFailToast: '传送带扩容失败，请重试',[\s\S]*?successToast: `传送带已扩容 \+\$\{this.getCapacityButtonIncrement\(\)\}`/,
    'manual conveyor expansion toast must match the level-specific reward',
);
assert.match(
    onCapacityAdTap,
