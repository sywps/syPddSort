...

{
    ...
    for (const testCase of cases) {
        ...
        assert.strictEqual((runtime.tapFeedbacks || []).length, 0, `${testCase.name} must not render a click halo for the corrected touch`);
...

{
    ...
    assert.strictEqual((runtime.tapFeedbacks || []).length, 0, 'a correct raw touch must not render a click halo');
...

{
    ...
    assert.strictEqual(endpoints.sourceHandVisible, false, 'the fallback source must skip a HUD-overlapping hand');
}

{
    ...
    assert.strictEqual(ripples.length, 0, 'smart hints must not render the retired blue tap rings');
