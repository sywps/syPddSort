...
assert.ok(homeScene.includes('68c7d0e7-b854-4fd7-903e-6176fb9aebbb@f9941'), 'Home.scene loading cover reference must use the deer image');
...
assert.ok(sceneRouter.includes('await this.waitForHomeScenePreloadIfNeeded();'), 'toHome must join an in-flight Home preload before doing a foreground route');
...
assert.ok(gameplaySession.includes("router.preloadHomeScene('gameplay-ready')"), 'Game ready must start Home.scene preload in the background');
