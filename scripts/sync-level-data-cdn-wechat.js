...
function parseLevelDataSourceFile(name) {
    const match = /^(zt_level_|level_|coop_level_)(\d+)\.json$/.exec(name);
...
function validateLevelDataPackage() {
    ...
    for (const pack of manifest.packs) {
        ...
        if (!['level_', 'zt_level_', 'coop_level_'].includes(packPrefix)) fail('level_live.json pack.prefix 不正确: ' + pack.url);
    ...
    for (const prefix of ['level_', 'zt_level_', 'coop_level_']) {
