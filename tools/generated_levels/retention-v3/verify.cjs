...
for(const id of [3,4,5]){
 assert.deepEqual(JSON.parse(fs.readFileSync(path.join(root,'assets/LevelData/level_'+id+'.json'),'utf8')),JSON.parse(fs.readFileSync(path.join(__dirname,'level_'+id+'.json'),'utf8')),'正式关卡应与已确认第三版一致');
