export const BEAN_SELECTION_EXPERIMENT_ID = 'bean_selection_ab_v1';
export const BEAN_SELECTION_EXPERIMENT_STORAGE = 'pdd.bean_selection_ab_v1.decision';
type Decision = {
...
export class BeanSelectionExperimentState {
    ...
    initialize(storage: Storage, isWechat: boolean, preview: boolean): void {
        ...
        if (preview || !isWechat) {
            this.decision = { id: BEAN_SELECTION_EXPERIMENT_ID, status: 'test', content: 'A', enrolledAt: 0, reason: 'preview' };
            return;
        }
        try {
            const raw = storage.getItem(BEAN_SELECTION_EXPERIMENT_STORAGE);
            if (raw) {
                const saved = JSON.parse(raw);
                if (saved.id !== BEAN_SELECTION_EXPERIMENT_ID || !['enrolled', 'excluded'].includes(saved.status)
            ...
            console.error('[BeanSelectionExperiment] invalid storage:', error);
            this.exclude('assignment_storage_unavailable');
        }
    }

    request(): Record<string, unknown> {
        return { id: BEAN_SELECTION_EXPERIMENT_ID, eligible: !this.decision && this.eligible,
    ...
    accept(openid: string, receipt: any): void {
        ...
        if (!openid || receipt?.id !== BEAN_SELECTION_EXPERIMENT_ID) { this.exclude('assignment_protocol_unavailable'); return; }
        ...
        this.verified = true;
        this.save({ id: BEAN_SELECTION_EXPERIMENT_ID, status: 'enrolled', content: receipt.content,
            enrolledAt: receipt.enrolledAt, reason: 'new_user', openid });
    }

    exclude(reason: string): void {
        if (this.locked || this.decision?.status === 'test' || this.decision?.status === 'excluded') return;
        this.save({ id: BEAN_SELECTION_EXPERIMENT_ID, status: 'excluded', content: 'A', enrolledAt: 0, reason });
    }
    ...
    private save(decision: Decision): void {
        try {
            ...
            this.storage.setItem(BEAN_SELECTION_EXPERIMENT_STORAGE, JSON.stringify(decision));
            this.decision = decision;
        } catch (error) {
            console.error('[BeanSelectionExperiment] cannot persist decision:', error);
            this.decision = { id: BEAN_SELECTION_EXPERIMENT_ID, status: 'excluded', content: 'A', enrolledAt: 0, reason: 'assignment_storage_unavailable' };
        }
    }
