...
const hash = async value => Array.from(new Uint8Array(await globalThis.crypto.subtle.digest('SHA-256', new TextEncoder().encode(value))), byte => byte.toString(16).padStart(2, '0')).join('');
const runId = async (post, user) => (await hash(`${post}:${user}`)).slice(0, 40);
const userId = async user => (await hash(user)).slice(0, 40);
...
async function newRun(post, owner, role, name, now) {
    return { id: await runId(post.id, owner), postId: post.id, owner, role, displayName: name,
...
function createCoopService(store, now = Date.now) {
    return async function execute(owner, event) {
        ...
        if (event.action === 'create') {
            ...
            return store.transaction(async tx => {
                ...
                if (user.activeCreated) {
                    ...
                    return { post: postView(post), run: runView(await readRun(tx, await runId(post.id, owner))) };
                }
                const createdAt = now();
                const post = { id: Array.from(globalThis.crypto.getRandomValues(new Uint8Array(12)), byte => byte.toString(16).padStart(2, '0')).join(''), levelId: event.levelId,
        ...
        return store.transaction(async tx => {
            ...
            const rid = await runId(id, owner);
            ...
            if (replay.board.isAllLocked()) {
                ...
                if (run.role === 'creator') post.creatorDone = true;
                    ...
                    const creatorUid = await userId(post.creator);
