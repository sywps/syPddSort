...
const COUNTDOWN_WARNING_TICK_SECONDS = new Set([33, 32, 31]);
...
export function installGameplayPlacementFxModule(target: any): void {
    ...
    Object.assign(target, {
        ...
        tickTimer() {
            ...
            const countdownWarningTickSecondsPlayed: Set<number> = this._countdownWarningTickSecondsPlayed
                || (this._countdownWarningTickSecondsPlayed = new Set<number>());
            if (COUNTDOWN_WARNING_TICK_SECONDS.has(this.timeRemain)
                && !countdownWarningTickSecondsPlayed.has(this.timeRemain)) {
                countdownWarningTickSecondsPlayed.add(this.timeRemain);
                AudioMgr.inst.play('tick');
            }
