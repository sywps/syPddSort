...
export function installThemePanelFlowModule(target: any): void {
    Object.assign(target, {
        ...
        loadThemeConfig(callback?: () => void) {
            ...
            console.log(`[ThemeConfig] remoteBundle=${!!this.remoteBundle}`);
            if (this.remoteBundle) {
                this.remoteBundle.load('themes', JsonAsset, (err, jsonAsset) => {
                    if (err) {
                        console.warn('[ThemeConfig] load themes from remoteBundle FAILED:', err.message);
                        onDone(null);
                    } else {
                        console.log('[ThemeConfig] themes loaded OK from remoteBundle');
                        onDone(jsonAsset.json);
                    }
                });
            } else {
                console.log('[ThemeConfig] loading remote bundle first...');
                assetManager.loadBundle('remote', (err, bundle) => {
                    if (err || !bundle) {
                        console.warn('[ThemeConfig] loadBundle remote FAILED:', err?.message);
                        onDone(null);
                        return;
                    }
                    console.log('[ThemeConfig] remote bundle loaded OK');
