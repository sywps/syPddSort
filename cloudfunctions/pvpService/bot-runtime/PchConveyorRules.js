...
exports.PchConveyorRules = void 0;
...
class PchConveyorRules {
    ...
    selectBoard(row, col) {
        var _a, _b;
        const preferredCorrectColor = ((_a = this.board.correctColors[row]) === null || _a === void 0 ? void 0 : _a[col]) || 0;
    ...
    storeBlock(block, entryCarrierIndex) {
    ...
    markQueuedBeansReady(count) {
        const nextReadyCount = Math.min(this.queuedColorIds.length, this.readyQueuedCount + Math.max(0, Math.floor(count)));
    ...
    autoPlaceTop(carrierIndex) {
        const stack = this.carriers[carrierIndex];
        const colorId = (stack === null || stack === void 0 ? void 0 : stack[stack.length - 1]) || 0;
        if (colorId <= 0)
            return this.emptyResult();
    ...
    autoPlaceAvailableLayers(carrierIndex) {
        const stack = this.carriers[carrierIndex];
        const boardCells = [];
        const carrierIndices = [];
        const colorIds = [];
        const sourceLayerIndices = [];
        if (!stack) {
            return {
                moved: 0,
                boardCells,
    ...
    forceCompleteColor(preferredColorId = 0) {
        ...
        for (let row = 0; row < this.board.height; row += 1) {
            for (let col = 0; col < this.board.width; col += 1) {
                ...
                if (colorId <= 0 || this.board.locked[row][col])
                    continue;
        ...
        for (let row = 0; row < this.board.height; row += 1) {
            for (let col = 0; col < this.board.width; col += 1) {
                ...
                if (current === colorId)
                    boardSources.push({ kind: 'board', row, col, colorId });
            }
        }
        ...
        const moves = sources.map((source, index) => ({ source, target: targets[index] }));
        const displacedColors = [];
        ...
        for (const target of targets) {
            ...
            if (displaced > 0 && displaced !== colorId)
                displacedColors.push(displaced);
            this.board.currentColors[target.row][target.col] = colorId;
            this.board.setLocked(target.row, target.col, true);
        ...
        displacedColors.forEach((displacedColor, index) => {
            ...
            this.board.setLocked(destination.row, destination.col, this.board.correctColors[destination.row][destination.col] === displacedColor);
    ...
    clearBufferToBoard() {
        ...
        for (const source of sources) {
            ...
            for (let row = 0; row < this.board.height; row += 1) {
                for (let col = 0; col < this.board.width; col += 1) {
                    ...
                    if (this.board.locked[row][col]
                        || claimed.has(key)
                        || this.board.correctColors[row][col] !== source.colorId)
                        continue;
        ...
        displacedColors.forEach((colorId, index) => {
            ...
            this.board.setLocked(destination.row, destination.col, this.board.correctColors[destination.row][destination.col] === colorId);
            changed.set(`${destination.row},${destination.col}`, destination);
        });
    ...
    topColor(carrierIndex) {
        const stack = this.carriers[carrierIndex];
        return (stack === null || stack === void 0 ? void 0 : stack[stack.length - 1]) || 0;
    }
    get carrierCount() {
        return this.carriers.length;
    }
    getCarrierCapacity(carrierIndex) {
        const normalizedIndex = this.normalizeCarrierIndex(carrierIndex);
    ...
    get conveyorSpeedMultiplier() {
        ...
        if (pendingTargetCounts.size === 0)
            return 1;
        const storedCounts = new Map();
        const returnableCarrierColors = new Set();
        for (const stack of this.carriers) {
        ...
        if (storedCounts.size !== pendingTargetCounts.size)
            return 1;
        for (const [colorId, pendingCount] of pendingTargetCounts) {
            if (storedCounts.get(colorId) !== pendingCount)
                return 1;
        }
        for (const colorId of returnableCarrierColors) {
            if (pendingTargetCounts.has(colorId))
                return 5;
        }
        return 1;
    }
    isBufferDeadlocked() {
        if (this.bufferCount !== this.bufferCapacity || this.entryCount > 0)
            return false;
        return !this.hasReturnableCarrierMatch();
    }
    shouldShowRedWarning(emptySlotThreshold) {
        ...
        if (this.bufferCapacity - this.bufferCount >= emptySlotThreshold)
            return false;
        return !this.hasReturnableCarrierMatch();
    }
    hasReturnableCarrierMatch() {
        const returnableCarrierColors = new Set();
        for (const stack of this.carriers) {
            for (const colorId of stack) {
                if (colorId > 0)
        ...
        if (returnableCarrierColors.size === 0)
            return false;
        for (let row = 0; row < this.board.height; row += 1) {
            for (let col = 0; col < this.board.width; col += 1) {
                if (this.board.currentColors[row][col] !== 0 || this.board.locked[row][col])
                    continue;
                if (returnableCarrierColors.has(this.board.correctColors[row][col]))
                    return true;
            }
        }
        return false;
...
exports.PchConveyorRules = PchConveyorRules;
